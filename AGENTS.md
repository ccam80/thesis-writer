...
| `src/output-styles/*.md` | Output styles; text between `<!-- shared:NAME start/end -->` is generated by `~/.claude/style-shared/sync.mjs` from its fragment of the same name, is stripped from `dist/`, and must be edited there rather than here |

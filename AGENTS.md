...
- A deferral, `[[what the author will supply]]`, records the author's decision to supply a value later. It is legal in any plan line, never originated by the agent to cover content it cannot state, and never `write-ready`; grounding resolves it before promotion and the writer refuses a block whose sentence points or element lines still carry one.
